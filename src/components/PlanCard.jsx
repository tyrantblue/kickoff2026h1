...
export function PlanCard({ maxKrs, okr }) {
  return (
    <article className="grid h-full grid-rows-[auto_auto_1fr] gap-[18px] rounded-lg border border-[var(--line)] bg-[var(--surface)] p-[22px] transition hover:-translate-y-1 hover:shadow-[0_16px_30px_rgba(15,23,42,0.1)]">
      ...
      <div className="grid auto-rows-fr gap-3.5">
        {okr.krs.map((kr) => (
          <div
            className="flex h-full flex-col gap-2.5 rounded-lg border border-[var(--card-muted-border)] bg-[var(--card-muted)] p-4 transition hover:translate-x-0.5 hover:border-[var(--brand)]"
            key={kr.title}
          >
            <strong className="text-[15px]">{kr.title}</strong>
            <p className="mt-auto leading-[1.7] text-[var(--text-soft)]">{kr.description}</p>
          </div>
        ))}
        {Array.from({ length: Math.max(0, maxKrs - okr.krs.length) }).map((_, index) => (
          <div
            aria-hidden="true"
            className="invisible flex h-full flex-col gap-2.5 rounded-lg border border-[var(--card-muted-border)] bg-[var(--card-muted)] p-4"
